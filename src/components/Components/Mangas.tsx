...
const Mangas = async ({ mangas }: { mangas: any }) => {
  const MangaCards = async () => {
    try {
      const mangaCardPromises = mangas?.map(async (manga: any, idx: number) => {
        const res: any = await fetchCover(manga.cover);
      ...
      // Wait for all manga card promises to resolve
      const mangaCards = await Promise.all(mangaCardPromises);

      // Return the resolved manga cards
      return mangaCards;
    } catch (error) {
      console.error("Error fetching manga cards:", error);
      return [];
    }
  };
