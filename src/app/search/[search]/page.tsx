...
const page = async ({ params }: any) => {
  const [popular, searchedManga] = await Promise.all([
    await fetchTopListings(),
    await searchManga(params?.search),
  ]);
  return (
    <main className="flex lg:flex-row flex-col py-2 gap-2 md:mx-5 mx-1 justify-center">
      <div className="w-full lg:w-9/12">
        <div className="text-lg py-5 font-semibold flex gap-1 items-center">
          <p className="opacity-70">Search Results For:</p>
